...
// Iteration #3.1 Bonus:
function sum(newSumArr) {
  let finalSum = 0;
  if (newSumArr.length > 0) {
    newSumArr.forEach(value => {
      if (typeof value === 'number') {
        finalSum += value;
      } else if (typeof value === 'string') {
        finalSum += value.length;
      } else if (typeof value === 'boolean') {
        if (value === true) {
          finalSum++;
        }
      } else {
        throw new Error('Unsupported data type submitted');
      }
    });
  }
  return finalSum;
...
function uniquifyArray(wordsArr) {
  if (wordsArr.length > 0) {
    for (let j = wordsArr.length - 1; j >= 0; j--) {
      if (wordsArr.indexOf(wordsArr[j]) !== j) {
        wordsArr.splice(j, 1);
      }
    }
  } else {
...
function howManyTimes(wordArr, checkWord) {
  let wordCount = 0;
...
function greatestProduct(matrix) {
  // running rows:
  // loop arrays
  // nested loop array values -> each loop stops after 16
  let product = 0;

  // checking product of rows
  matrix.forEach(array => {
    for (let i = 0; i < array.length - 3; i++) {
      let rowMultiplication =
        array[i] * array[i + 1] * array[i + 2] * array[i + 3];
      if (rowMultiplication > product) {
        product = rowMultiplication;
      }
    }
  });
